...
describe("UR Test Case 2: ", function () {
  let Whiskey;
  let Water;
  let USDC;
  let DAI;
  ...
  describe("Testing", function () {
    ...
    it("Deposit 100 USDC on Water", async function () {
      console.log('UR after deposit 100 USDC on Water: (expect)', 0);
      await USDC.connect(user1).approve(Water.address, depositAmount);
      await Water.connect(user1).depositStableCoin(USDC.address, 100e6, user1.address);
      expect(await Water.utilizationRate()).to.equal(0);
      console.log('UR after deposit 100 USDC on Water: (result)', await Water.utilizationRate());
    });
    it("Deposit 30 USDC on Whiskey", async function () {
      console.log('UR after deposit 30 USDC on Whiskey: (expect)', '6000 (60%)');
      await USDC.connect(user1).approve(Whiskey.address, depositAmount);
      await Whiskey.connect(user1).deposit(USDC.address, 30e6, 200);
      console.log('UR after deposit 30 USDC on Whiskey: (result)', await Water.utilizationRate());
    });
    it("Withdraw 10 USDC from Water", async function () {
      console.log('UR after withdraw 10 USDC from Water: (expect)', '6670 (66.7%)');
      await Water.connect(user1).withdrawStableCoin(USDC.address, 10e6, user1.address, user1.address);
      console.log('UR after withdraw 10 USDC from Water: (result)', await Water.utilizationRate());
    });
    it("Withdraw 10 USDC from Whiskey", async function () {
      console.log('UR after withdraw 10 USDC from Water: (expect)', '4400 (44%)');
      const withdrawAmount = hre.ethers.utils.parseEther("30");
      await Whiskey.connect(user1).makeWithdrawRequest(USDC_address, withdrawAmount);
      // Increase time and go to next epoch
      await startNewEpoch();
      await startNewEpoch();
      await startNewEpoch();
      await Whiskey.connect(user1).withdrawStableCoin(USDC.address, 30e6);
      console.log('UR after withdraw 10 USDC from Water: (result)', await Water.utilizationRate());
    });
  });
});
